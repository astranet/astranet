...
func (prev PrimaryLayer) addAll(nextLock lockie.Lockie, nextIter *PrimaryEnumerator,
	onAdd func(key1 uint64, key2 RouteInfo)) {
	...
	for err != io.EOF {
		if nextLayer.Len() > 0 {
			...
			if onAdd != nil {
				newLayer.Sync(nextLayer, func(k2 RouteInfo) {
					onAdd(nextK, k2)
				}, nil)
			} else {
...
func (prev PrimaryLayer) deleteAll(prevIter *PrimaryEnumerator,
	onDel func(key1 uint64, key2 RouteInfo)) {
	...
	for err != io.EOF {
		...
		if onDel != nil {
			prevLayer.Sync(NewSecondaryLayer(prev.cmp2), nil, func(k2 RouteInfo) {
				onDel(prevK, k2)
			})
		} else {
...
func (prev PrimaryLayer) syncAll(next PrimaryLayer, prevIter, nextIter *PrimaryEnumerator,
	onAdd, onDel func(k1 uint64, k2 RouteInfo)) {
	...
	for {
		...
		case nextErr == io.EOF:
			...
			if onDel != nil {
				prevLayer.ForEach(func(k2 RouteInfo, v2 *FinalizerList) bool {
					if onDel != nil {
						onDel(prevK, k2)
					}
					if v2 != nil {
		...
		default:
			...
			case onAdd != nil && onDel != nil:
				prevLayer.Sync(nextLayer, func(k2 RouteInfo) {
					onAdd(nextK, k2)
				}, func(k2 RouteInfo) {
					onDel(prevK, k2)
				})
			...
			case onDel != nil:
				prevLayer.Sync(nextLayer, nil, func(k2 RouteInfo) {
					onDel(prevK, k2)
				})
